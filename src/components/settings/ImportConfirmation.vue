<template>
  <Dialog :open="open" @clickOutside="close">
    ...
    <div class="d-flex flex-middle my16">
    ...
    <div v-if="panes.length" class="form-group mt16">
      <label>Markets</label>
      <button v-for="(market, index) in markets" :key="index" type="button" disabled class="btn -green -small -outline mr4 mb4">
        {{ market }}
      </button>
    </div>
    <footer>
      <a href="javascript:void(0);" class="btn -text" @click="close(false)">Cancel</a>
      <button class="btn -large" @click="close(workspace)">IMPORT & USE</button>
    </footer>
  </Dialog>
</template>
